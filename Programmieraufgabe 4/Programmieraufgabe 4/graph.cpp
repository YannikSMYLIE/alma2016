...
{
	if (tail >= num_nodes() || tail < 0 || head >= num_nodes() || head < 0) {
...
{
	if (node < 0 || node >= static_cast<int>(_nodes.size())) {
...
int Graph::deepsearch(NodeId goal) const
{
	Graph::Node curNode;
	int zhk = 0;
	...
	for (int i = 0; i < num_nodes(); i++)
	{
		...
		{
			zhk++;
			deepsearchVisit(i, goal);
		}
	}
	return zhk;
}
Graph::Node Graph::deepsearchVisit(NodeId nodeID, NodeId goal) const
{
	Graph::Node node = get_node(nodeID);
	Graph::Node neighborNode;
	for (int i = 0; i < node.adjacent_nodes().size(); i++)
	{
		neighborNode = get_node(node.adjacent_nodes()[i].id);
		if (!neighborNode.marked())
			deepsearchVisit(node.adjacent_nodes()[i].id, goal);
	}
